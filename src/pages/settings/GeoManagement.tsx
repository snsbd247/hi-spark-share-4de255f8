...
import { Plus, Trash2, MapPin, Globe, Building, Map } from "lucide-react";
...
function DivisionsTab() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["geo-divisions"] }); setName(""); toast.success("Division added"); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["geo-divisions"] }); toast.success("Division deleted"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        <TableHeader><TableRow><TableHead>Name</TableHead><TableHead className="w-20">Action</TableHead></TableRow></TableHeader>
        <TableBody>
          {divisions?.map((d: any) => (
            <TableRow key={d.id}>
              <TableCell>{d.name}</TableCell>
              <TableCell><Button variant="ghost" size="icon" onClick={() => setDeleteId(d.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
            </TableRow>
          ))}
...
function DistrictsTab() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["geo-districts"] }); setName(""); toast.success("District added"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["geo-districts"] }); toast.success("District deleted"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        <TableHeader><TableRow><TableHead>District</TableHead><TableHead>Division</TableHead><TableHead className="w-20">Action</TableHead></TableRow></TableHeader>
        <TableBody>
          {districts?.map((d: any) => (
            <TableRow key={d.id}>
              <TableCell>{d.name}</TableCell>
...
function UpazilasTab() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["geo-upazilas"] }); setName(""); toast.success("Upazila added"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["geo-upazilas"] }); toast.success("Upazila deleted"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        <TableHeader><TableRow><TableHead>Upazila</TableHead><TableHead>District</TableHead><TableHead className="w-20">Action</TableHead></TableRow></TableHeader>
        <TableBody>
          {upazilas?.map((u: any) => (
            <TableRow key={u.id}>
              <TableCell>{u.name}</TableCell>
              <TableCell><Badge variant="secondary">{u.geo_districts?.name}</Badge></TableCell>
              <TableCell><Button variant="ghost" size="icon" onClick={() => setDeleteId(u.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
            </TableRow>
          ))}
...
function ZonesTab() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["zones"] }); setName(""); setAddress(""); toast.success("Zone added"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        <TableHeader><TableRow><TableHead>Zone / Area</TableHead><TableHead>Address</TableHead><TableHead>Status</TableHead><TableHead className="w-20">Action</TableHead></TableRow></TableHeader>
        <TableBody>
          {zones?.map((z) => (
            <TableRow key={z.id}>
              <TableCell className="font-medium">{z.area_name}</TableCell>
              <TableCell>{z.address || "-"}</TableCell>
              <TableCell><Badge variant={z.status === "active" ? "default" : "secondary"}>{z.status}</Badge></TableCell>
              <TableCell><Button variant="ghost" size="icon" onClick={() => setDeleteId(z.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
            </TableRow>
          ))}
