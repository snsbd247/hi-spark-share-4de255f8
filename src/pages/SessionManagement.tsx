...
import {
  Monitor, Smartphone, Globe, LogOut, Shield, Loader2,
  ...
  const terminateAllMut = useMutation({
    mutationFn: async () => {
      const otherSessions = sessions.filter(
        (s: any) => s.session_token !== currentToken && s.status === "active"
      );
      for (const s of otherSessions) {
        await db.from("admin_sessions").update({ status: "terminated" }).eq("id", s.id);
      }
    },
    onSuccess: () => {
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              <Shield className="h-6 w-6" /> Session Management
            </h1>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Currently logged in sessions</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                    ...
                    >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {safeFormat(session.updated_at, "dd MMM HH:mm")}
                            </span>
                          </div>
                        </div>
                      </div>
                      {!isCurrent && (
                        <Button
                          ...
                          onClick={() => setConfirmDialog(session.id)}
                        >
              ...
              <Table>
                ...
                <TableBody>
                  {inactiveSessions.slice(0, 20).map((s: any) => (
                    <TableRow key={s.id}>
                      <TableCell className="text-sm">{s.device_name || "Unknown"}</TableCell>
                      <TableCell className="text-sm">{s.browser || "Unknown"}</TableCell>
                      ...
                      <TableCell className="text-sm text-muted-foreground">
                        {safeFormat(s.updated_at, "dd MMM yyyy HH:mm")}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
      ...
      <Dialog open={!!confirmDialog} onOpenChange={(o) => !o && setConfirmDialog(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            This will immediately log out the device. The user will need to login again.
          </p>
          <div className="flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setConfirmDialog(null)}>Cancel</Button>
            <Button
              variant="destructive"
              onClick={() => confirmDialog && terminateMut.mutate(confirmDialog)}
